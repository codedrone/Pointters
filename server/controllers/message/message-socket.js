...
module.exports =  async (d, socket)=>{
    ...
    const user = await findOneUser({ _id: ObjectId(message.userId) });
...
>>>>>>> develop/ling
    ...
    if(newMessage.serviceId){
      const service = await findOneService({ _id: newMessage.serviceId });
      if(service) {
    ...
    if(newMessage.offerId){
      const offer = await findOneOffer({ _id: newMessage.offerId });
      if(offer) {
    ...
    if(newMessage.requestId){
      ...
      const request = await findOneRequest({ _id: newMessage.requestId });
      if(request) {
